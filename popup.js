...
async function getVideoDescriptions(channelId, numberOfVideos) {
    ...
    try {
      ...
      const descriptions = videosData.items.map(item => item.snippet.description);
  
      // write descriptions to description.html
      const file = new File([descriptions.join("<br>")], "description.html", { type: "text/html" });
...
async function handleSubmit(event) {
  ...
  if (channelId) {
    ...
    if (videoDescriptions.length > 0) {
      messageDiv.innerHTML = videoDescriptions.join('<br>');
    } else {
